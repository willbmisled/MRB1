v='MRB1_InOutModelsCompare20110711.r'

#NOTE: this is a work in progress.  Considering adding a seasonal factor to the equations.
...
model<-'H1'
nl<-c();Yhat<-c();rmse<-NA;aic<-NA;R2<-NA;adjR2<-NA  #reset values 
...
model<-'H1s'  #with Seasonal Adjustment
nl<-c();Yhat<-c();rmse<-NA;aic<-NA;R2<-NA;adjR2<-NA  #reset values 
nl<- nlrob(log10(Y) ~ S*log10(X/(1+(c1*hrt))),
  start=list(c1 = .45,S=1), 
  data=In1,algorithm = "default",  trace=F,na.action = na.exclude) 
    rmse<-sqrt(sum(na.exclude(nl$residuals**2))/length(na.exclude(nl$residuals)))
    aic<-AIC(nl)
    Yhat=predict(nl, newdata = In1)
...
model<-'H2s'  #with Seasonal Adjustment
nl<-c();Yhat<-c();rmse<-NA;aic<-NA;R2<-NA;adjR2<-NA  #reset values
nl<- nlrob(log10(Y) ~ S*log10(X/(1+c1)),
  start=list(c1 = 1.06,S=1), 
  data=In1,algorithm = "default",  trace=F,na.action = na.exclude) 
    rmse<-sqrt(sum(na.exclude(nl$residuals**2))/length(na.exclude(nl$residuals)))
    aic<-AIC(nl)
    Yhat=predict(nl, newdata = In1)
...
model<-'H3'  #with Seasonal Adjustment
nl<-c();Yhat<-c();rmse<-NA;aic<-NA;R2<-NA;adjR2<-NA  #reset values
nl<- nlrob(log10(Y) ~ S*log10(X/(1+((c1/Z)*hrt))),
  start=list(c1 = 5.1,S=1), 
  data=In1,algorithm = "default",  trace=F,na.action = na.exclude) 
    rmse<-sqrt(sum(na.exclude(nl$residuals**2))/length(na.exclude(nl$residuals)))
    aic<-AIC(nl)
    Yhat=predict(nl, newdata = In1)
...
model<-'H4s'   #with Seasonal Adjustment
nl<-c();Yhat<-c();rmse<-NA;aic<-NA;R2<-NA;adjR2<-NA  #reset values 
nl<- nlrob(log10(Y) ~ S*log10(X/(1+(c1*hrt**c2))),
  start=list(c1 = 1.12,c2=-.53,S=1), 
  data=In1,algorithm = "default",  trace=F,na.action = na.exclude) 
    rmse<-sqrt(sum(na.exclude(nl$residuals**2))/length(na.exclude(nl$residuals)))
    aic<-AIC(nl)
    Yhat=predict(nl, newdata = In1)
...
model<-'H5s'
nl<-c();Yhat<-c();rmse<-NA;aic<-NA;R2<-NA;adjR2<-NA  #reset values
nl<- nlrob(log10(Y) ~ S*log10((c1*X)/(1+(c2*hrt))),
  start=list(c1 = .65,c2=.03), 
  data=In1,algorithm = "default",  trace=F,na.action = na.exclude) 
rmse<-sqrt(sum(na.exclude(nl$residuals**2))/length(na.exclude(nl$residuals)))
aic=AIC(nl)
...
model<-'H6'
nl<-c();Yhat<-c();rmse<-NA;aic<-NA;R2<-NA;adjR2<-NA  #reset values
nl<- nlrob(log10(Y) ~ log10(X/(1+(c1*hrt**c2*Z**c3*X**.5))),
  start=list(c1 = 12.26, c2 = .45, c3=-.16),
  data=In1,algorithm = "default",  trace=T,na.action = na.exclude)
    rmse<-sqrt(sum(na.exclude(nl$residuals**2))/length(na.exclude(nl$residuals)))
    aic<-AIC(nl)
    Yhat=predict(nl, newdata = In1)
    R2<-summary(lm(log10(In1$Y)~Yhat))$r.squared
...
model<-'H6s'   #with Seasonal Adjustment
nl<-c();Yhat<-c();rmse<-NA;aic<-NA;R2<-NA;adjR2<-NA  #reset values
nl<- nlrob(log10(Y) ~ S*log10(X/(1+(c1*hrt**c2*Z**c3*X**.5))),
  start=list(c1 = 12.26, c2 = .45, c3=-.16,S=1),
  data=In1,algorithm = "default",  trace=T,na.action = na.exclude)
    rmse<-sqrt(sum(na.exclude(nl$residuals**2))/length(na.exclude(nl$residuals)))
    aic<-AIC(nl)
    Yhat=predict(nl, newdata = In1)
...
model<-'H7'
nl<-c();Yhat<-c();rmse<-NA;aic<-NA;R2<-NA;adjR2<-NA  #reset values
nl<- nlrob(log10(Y) ~ log10(c1*X*hrt**c2*Z**c3),
  start=list(c1 =.27, c2 = -.22, c3=.16),
  data=In1,algorithm = "default",  trace=F,na.action = na.exclude)
    rmse<-sqrt(sum(na.exclude(nl$residuals**2))/length(na.exclude(nl$residuals)))
    aic<-AIC(nl)
    Yhat=predict(nl, newdata = In1)
...
model<-'H7s'
nl<-c();Yhat<-c();rmse<-NA;aic<-NA;R2<-NA;adjR2<-NA  #reset values
nl<- nlrob(log10(Y) ~ S*log10(c1*X*hrt**c2*Z**c3),
  start=list(c1 =.27, c2 = -.22, c3=.16),
  data=In1,algorithm = "default",  trace=F,na.action = na.exclude)
    rmse<-sqrt(sum(na.exclude(nl$residuals**2))/length(na.exclude(nl$residuals)))
    aic<-AIC(nl)
