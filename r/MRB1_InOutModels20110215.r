v='MRB1_InOutModels20110215.r' #version = rscript file name
...
PlotNL<-function(nl,X,Y,Label,Title,AXES){
 ...
 #Get axis limits & Labels
  ...
  #Plot model values
    ...
    abline(0,1,lwd=2,col="blue")
    title(main=Title,cex.main=1,sub=v,cex.sub=.7)
    legend("topleft",c(paste('R2=',round(summary(lm(Y~Yhat))$r.squared,3)),
        paste('adjR2=',round(summary(lm(Y~Yhat))$adj.r.squared,3)),
        paste('rmse=',round(rmse,3)),paste('aic=',round(aic,3)),
...
###########################   
#save the data
#save(LMN,nln,LMP,nlp,file='M:/Net MyDocuments/EPA/Data/RData/InOut_20110215.rda')
