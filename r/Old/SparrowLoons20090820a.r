...
library(vioplot)
BP_Prob=function(stress,response, SL1,SL2,xlabel,ylabel)
{
...
boxplot(LowDist, MedDist, HighDist,notch=TRUE, width=c(sqrt(length(Low)),sqrt(length(Med)),sqrt(length(High))),
   ylim=c(0,1),names=c(paste("<",round(SL1,1)),round((SL1+SL2)/2,1), paste(">",round(SL2,1))),xlab=xlabel,range=0,ylab=ylabel)    
polygon(x=c(.4,.4, 3.6,3.6),
        y=c(ucp[1],ucp[3],ucp[3],ucp[1]), density=NA, col="azure", border=NA)
par(new=T) 
boxplot(LowDist, MedDist, HighDist,notch=TRUE, width=c(sqrt(length(Low)),sqrt(length(Med)),sqrt(length(High))),
   ylim=c(0,1),names=c(paste("<",round(SL1,1)),round((SL1+SL2)/2,1), paste(">",round(SL2,1))),xlab=xlabel,range=0,ylab=ylabel)    
...
BP_Prob(log10(Nin),SumOf.Nesting.Pairs,log10(350),log10(750),
     "Log10 Nitrogen Concentration from SPARROW","Probability of Presence of Nesting Loons")  

BP_Prob(log10(Nin),SumOf.Paired.Adults+SumOf.Unpaired.Adults,log10(350),log10(750),
     "Log10 Nitrogen Concentration from SPARROW","Probability of Presence Loons")  
     
BP_Prob(log10(Pin),SumOf.Nesting.Pairs,log10(10),log10(25),
     "Log10 Phosphorus Concentration from SPARROW","Probability of Presence of Nesting Loons")  

BP_Prob(log10(Pin),SumOf.Paired.Adults+SumOf.Unpaired.Adults,log10(10),log10(25),
     "Log10 Phosphorus Concentration from SPARROW","Probability of Presence Loons")  
