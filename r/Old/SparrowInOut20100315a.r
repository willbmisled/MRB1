...
get <- sqlQuery(con, "   
SELECT MRB1_WBIDLakes.WB_ID, tblJoinNLAID_WBID.NLA_ID, MRB1_WBIDLakes.AlbersAreaM AS Area, NLAMRB1PredDepth20100315.nlamaxdepth AS Zmax_NLA, NLAMRB1PredDepth20100315.CorrectedPredDepth AS Zmax_GIS, tblSparrowLoads.InflowM3_yr AS Inflow, tblSparrowLoads.OutflowM3_yr AS Outflow, tblNLA_WaterQualityData.NTL, tblSparrowLoads.N_Load_kg_yr AS NInput, tblSparrowLoads.N_Output AS NOutput, tblNLA_WaterQualityData.PTL, tblSparrowLoads.P_Load_kg_yr AS PInput, tblSparrowLoads.P_Output AS POutput, tblNLA_WaterQualityData.CHLA AS ChlA, tblNLA_WaterQualityData.SECMEAN AS Secchi
...
test=summary(lm(log10(TN)~Nhat))
test
plot(Nhat, log10(TN), xlab="Bachman Model Predicted Log TN", ylab="NLA Measured Log10 TP")
abline(test, lwd=2)
title(main = paste("log10((Nin)/(1+((",round(estimate$coefficients[1],2),"*hrt**",round(estimate$coefficients[2],2),")*hrt)))"),
sub=paste('Predicted Zmax; r-squared=',round(test$r.squared,4)))
