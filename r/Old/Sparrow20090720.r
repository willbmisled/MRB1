...
#Get thresholds for CP_out and CN to assign trophic state.

summary(lm(logCP_out~logTP))
#logCP_out=.49437*(logTP)+.96660

10**(.49437*log10(10)+.96660)
10**(.49437*log10(25)+.96660)
10**(.49437*log10(50)+.96660)


summary(lm(logCN~logTN))
#logCN=.6010*(logTN)+1.3511

10**(.6010*(log10(350))+1.3511)
10**(.6010*(log10(750))+1.3511)
10**(.6010*(log10(1400))+1.3511)

#Convert Sparrow Concentration to TP and TN

summary(lm(logTP~logCP_out))
#logTP=(.717398*logCP_out)+.005966

summary(lm(logTN~logCN))
#logTN=(.68081*logCN)+.59939
Est_logTN=((.68081*log10(CN))+.59939)

plot(logCN,logTN)
abline(lm(logTN~logCN))
points(logCN,Est_logTN,col='red')

plot(logTN,Est_logTN)
abline(lm(Est_logTN~logTN))

plot(fitted.values(lm(logTN~logCN)),Est_logTN)
...
Est_logCHLA=(1.0241163*logTP)+(.0031017*NPRatio)-.4976196
Est_logSECMEAN=(-.383974*logCHLA)+(.232819*logTN)+(-.383656*logTP)+(-.001006*NPRatio)+.479130
Est_TP=10**Est_logTP
Est_TN=10**Est_logTN
