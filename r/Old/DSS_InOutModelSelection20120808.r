...
v='DSS_InOutModelSelection20120808.r'
...
con <- odbcConnectAccess("C:/Bryan/EPA/Data/WaterbodyDatabase/WaterbodyDatabase.mdb")
NLA<- sqlQuery(con, "
SELECT tblJoinNLAID_WBID.WB_ID, tblJoinNLAID_WBID.NLA_ID, NLA2007Sites_DesignInfo.SITE_TYPE, tblNLA_WaterQualityData.VISIT_NO, NLA2007Sites_DesignInfo.LAKE_SAMP, tblJoinNLAID_WBID.Rank, tblNLA_WaterQualityData.NTL, tblNLA_WaterQualityData.PTL, tblNLA_WaterQualityData.CHLA, tblNLA_WaterQualityData.SECMEAN, tblNLA_WaterQualityData.CLEAR_TO_BOTTOM
FROM (tblJoinNLAID_WBID INNER JOIN NLA2007Sites_DesignInfo ON tblJoinNLAID_WBID.NLA_ID = NLA2007Sites_DesignInfo.SITE_ID) INNER JOIN tblNLA_WaterQualityData ON (NLA2007Sites_DesignInfo.SITE_ID = tblNLA_WaterQualityData.SITE_ID) AND (NLA2007Sites_DesignInfo.VISIT_NO = tblNLA_WaterQualityData.VISIT_NO)
...
#Eliminate Lake Champlain lakes where SPARROW predictions Nin doesn't equal Nout (within 1mg/l)      n=17,982
MRB1<-One[round(One$Nin)==round(One$Nout) & One$WB_ID!=22302965,]

MRB1<-MRB1[,c(1,2,7:12,19:29)] #eliminate unnecessary fields

#Select the NLA data only from MRB1  n=134
...
ModelSearch<-function(MRB1In,MRB1Out,NLAobs,Data){
    ...
    #Reckhow(Backmann) Pers. Comm. H4se  log10(TN)=log10(Nin/(1+(0.693*hrt^0.45))) #NE
        tryCatch({a<- nlrob(log10(Y) ~ log10(Xin/(1+(c1*hrt^c2))),
            start=list(c1 = 0.693,c2=0.45),
            data=A,algorithm = "default",  trace=F,na.action = na.exclude)
            keep<-rbind(keep,Stats(a,A,NLAobs,MRB1In,'H4se'))
             } , error = function(e) { print("H4se")  })
    #Reckhow(Backmann) Pers. Comm. H4ne  log10(TN)=log10(Nin/(1+(0.67*hrt^0.25))) #SE
        tryCatch({a<- nlrob(log10(Y) ~ log10(Xin/(1+(c1*hrt^c2))),
...
#save the data
#
save(LMN,nln,LMP,nlp,MRB1,NLA,file='C:/Bryan/EPA/Data/RData/InOutModelSelection20120808.rda')
