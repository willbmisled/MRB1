v='DSS_InOutModelsCompare20120802.r'
...
ModelSearch<-function(MRB1In,MRB1Out,NLAobs,Data){
    ...
    #Ken Reckhow Eutromod H6modified: log10(TP)=log10(Pin/(1+(12.26*hrt**.45*z**-.16*Pin**.5)))  see Reckhow_NE lakes - Eutromod - page1.pdf
      ...
      #mg/l
        tryCatch({a<- nlrob(log10(Y) ~ log10(Xin/(1+(c1*hrt**c2*Zmean**c3*Xin**.5))),
            ...
            data=A,algorithm = "default",  trace=F,na.action = na.exclude)
            keep<-rbind(keep,Stats(a,A,NLAobs,MRB1In,'H6m'))
             } , error = function(e) { print("H6m")  })
