...
MRB1 <- sqlQuery(con, "   
SELECT MRB1_WBIDLakes.WB_ID, tblJoinNLAID_WBID.NLA_ID, Round([Centroid_Long]) AS Lon, Round([Centroid_Lat]) AS Lat, tblSparrowLoads.OutflowM3_yr AS Flow, Round([MeanElevationM]) AS Elevation, Round([ShorelineAlbersM]) AS Shoreline, Round([AlbersAreaM]) AS Area, Round(100000*[BASINAREA]) AS Basin, Round([Basin]/[Area],2) AS BasinAreaRatio, Round([Shoreline]/(((3.1416*[Area])^0.5)*2),2) AS SDI
FROM tblSparrowLoads INNER JOIN ((tblNED_WBID_Elevation INNER JOIN ((MRB1_WBIDLakes INNER JOIN tblJoinNLAID_WBID ON MRB1_WBIDLakes.WB_ID = tblJoinNLAID_WBID.WB_ID) INNER JOIN tblNLA_WaterQualityData ON tblJoinNLAID_WBID.NLA_ID = tblNLA_WaterQualityData.SITE_ID) ON tblNED_WBID_Elevation.WB_ID = MRB1_WBIDLakes.WB_ID) INNER JOIN NLA_National_Basin ON tblJoinNLAID_WBID.NLA_ID = NLA_National_Basin.SITEID) ON tblSparrowLoads.WB_ID = MRB1_WBIDLakes.WB_ID
...
logVARS<-log10(MRB1[,3:11]+1)
loglogVARS<-log(log(MRB1[,3:11]+1)+1)
sqrtVARS<-sqrt(data.frame(-MRB1[,3],MRB1[,4:11]))
...
vars<-data.frame(MRB1[,3:4],Flow=log(log(MRB1[,5]+1)+1),Elevation=sqrt(MRB1[,6]),
                 Shoreline=log(log(MRB1[,7]+1)+1),Area=log(log(MRB1[,8]+1)+1),
                 Basin=log(log(MRB1[,9]+1)+1),BARatio=log(log(MRB1[,10]+1)+1),SDI=log(log(MRB1[,11]+1)+1))
...
#Principal Components Analysis
  ...
  pca1 <- dudi.pca(df=vars,center=TRUE,scale=TRUE,scannf=FALSE,nf=9)
