...
summary(VfCal_N,na.rm=T)   #median=5.477 #Harrison et al 1990 median value = 6.83
NRH<-round((1-exp(-median(VfCal_N,na.rm=T)/MRB1$HL)),2)
summary(NRH)   #median=21%
#Nin adjusted for removal
N_NR<-MRB1$Nin*(1-NRH)
summary(N_NR)

#Total N removed by NE Lakes
sum(MRB1$Ninput*NRH) # 13.5 B kilos 
...
NRQ[NRQ>1]<-1
summary(NRQ) #median=34%
#Total N removed by NE Lakes
sum(MRB1$Ninput*NRQ) # 18.2 B kilos 
...
NRA<-1-(1/(1+(9.9*(MRB1$HL**-1))))
  summary(NRA)
#Total N removed by NE Lakes
sum(MRB1$Ninput*NRA) # 18 B kilos (.025 Teragrams or 25 Gigagrams)
