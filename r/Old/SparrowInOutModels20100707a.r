...
nln<- nlrob(log10(Y) ~ log10((X)/(1+(c1*hrt**c2))),
  ...
  data=In1,algorithm = "default",  trace=F,na.action = na.exclude)
    ...
    aic<-AIC(nln)
    Yhat=predict(nln, newdata = In1)
TitleNL<-bquote(log[10]*phantom(.)*.(names(In[2]))==log[10]*phantom(.)*(frac(.(names(In[4])),1+
    .(round(nln$coefficients[1],2))*hrt*phantom(.)^.(round(nln$coefficients[2],2))*phantom(.))))
PlotModel(log10(In1$X),log10(In1$Y),Yhat,rmse,aic,group,Colors,TitleNL)
######################   
...
plot(hrt,nln$residuals,pch=19,col=group,xlim=c(0,5),ylim=c(-.7,.8),
  ...
  xlab='Hydraulic Residence Time (years)',ylab='Residuals',main=TitleNL)
  abline(h=0,lwd=2,col="blue")
  legend("bottomright",c("Short","Med","Long"),pch=19,cex=1,col=Colors,bty='n')
  text(2.75,.68,'MRB1 Model UNDER-estimates Nitrogen')
  text(2.75,-.6,'MRB1 Model OVER-estimates Nitrogen')
#####################################
...
In1<-data.frame(Y=In[,2],X=In[,4],hrt=In[,5],Z=In[,6])
nlp<- nlrob(log10(Y) ~ log10(X/(1+((c1*(hrt**c2)*(Z**c3)*(X**c4))*hrt))),
  start=list(c1 = 12.26, c2 = -.55, c3=-.16,c4=.5),
  data=In1,algorithm = "default",  trace=F,na.action = na.exclude)
    rmse<-sqrt(sum(na.exclude(nlp$residuals**2))/length(na.exclude(nlp$residuals)))
    aic<-AIC(nlp)
    Yhat=predict(nlp, newdata = In1)
TitleNL<-bquote(log[10]*.(names(In[2]))==log[10]*(frac(.(names(In[4])),1+
    .(round(nlp$coefficients[1],1))*hrt*phantom(.)^.(round(nlp$coefficients[2],1))*phantom(.)*
    z*phantom(.)^.(round(nlp$coefficients[3],1))
    *.(names(In[4]))*phantom(.)^.(round(nlp$coefficients[4],1))*phantom(.)*hrt)*phantom(.)))
PlotModel(log10(In1$X),log10(In1$Y),Yhat,rmse,aic,group,Colors,TitleNL)
######################   
...
plot(hrt[!is.na(In[,2])],LMP$residuals,pch=19,col=group[!is.na(In[,2])],xlim=c(0,5),ylim=c(-1.2,1.4),
  ...
  abline(h=0,lwd=2,col="blue")
  legend("topright",c("Short","Med","Long"),pch=19,cex=1,col=Colors,bty='n')
  text(2.75,1.2,'MRB1 Model UNDER-estimates Phosphorus')
  text(2.75,-1,'MRB1 Model OVER-estimates Phosphorus')
