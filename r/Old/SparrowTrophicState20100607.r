...
a<-spsurvey.analysis(sites=Sites,design=Design,data.cont=DataCont)
b<-cont.analysis(spsurvey.obj=a)
par(mfrow=c(1,1))
cdf.plot(b$CDF,logx="x",xlbl=paste('log10(',unique(b$CDF$Indicator),')'))


#homemade solution
pp<-data.frame(Nugl=sort(log10(Nugl)),percent=100*seq(1:length(na.exclude(Nugl)))/length(na.exclude(Nugl)))
plot(log10(b$CDF$Value),b$CDF$Estimate.P,xlim=c(0,5),type='l',col='green',lwd=2)
lines(pp[,1],pp[,2])

#use ecdf to get the cdf for MRB1

require(hmisc)
plot(ecdf(log10(Nugl)),lwd=2,col='red')    
