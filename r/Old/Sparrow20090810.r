...
plot(log10(Pout), log10(TP), xlab="Sparrow Observed Log Phosporus Outflow Concentration", ylab="NLA Measured Log Total Phosporus")
...
#test Eutromod coefficients with Sparrow data
#NLA versus Outflow Concentration
Phat=log10(Pout/(1+((c1*(hrt**c2)*(z**c3)*(Pout**c4))*hrt)))
test=summary(lm(log10(TP)~Phat))
test
...
#NLA versus Outflow Concentration
library(robustbase)
estimate <- nlrob(log10(TP) ~ log10(Pout/(1+((c1*(hrt**c2)*(z**c3)*(Pout**c4))*hrt))),
                     start=list(c1 = 12.26, c2 = -.55, c3=-.16,c4=.5),
                     data=MRB1,algorithm = "default",  trace=T) 
