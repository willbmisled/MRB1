...
MRB1 <- sqlQuery(con, "   
SELECT MRB1_WBIDLakes.WB_ID, tblJoinNLAID_WBID.NLA_ID, MRB1_WBIDLakes.AlbersAreaM AS Area, NLA2007Sites_DesignInfo.DEPTHMAX AS ZMAX, tblSparrowLoads.InflowM3_yr AS Inflow, tblSparrowLoads.OutflowM3_yr AS Outflow, tblNLA_WaterQualityData.NTL, tblSparrowLoads.N_Load_kg_yr AS NInput, tblSparrowLoads.N_Output AS NOutput, NLA_DIATOM_INFERRED_CHEM.NTL_INF_TOP AS Ninf, tblNLA_WaterQualityData.PTL, tblSparrowLoads.P_Load_kg_yr AS PInput, tblSparrowLoads.P_Output AS POutput, NLA_DIATOM_INFERRED_CHEM.PTL_INF_TOP AS Pinf, tblNLA_WaterQualityData.CHLA AS ChlA, tblNLA_WaterQualityData.SECMEAN AS Secchi
FROM ((((MRB1_WBIDLakes INNER JOIN tblJoinNLAID_WBID ON MRB1_WBIDLakes.WB_ID = tblJoinNLAID_WBID.WB_ID) INNER JOIN tblSparrowLoads ON MRB1_WBIDLakes.WB_ID = tblSparrowLoads.WB_ID) INNER JOIN NLA2007Sites_DesignInfo ON tblJoinNLAID_WBID.NLA_ID = NLA2007Sites_DesignInfo.SITE_ID) INNER JOIN tblNLA_WaterQualityData ON (NLA2007Sites_DesignInfo.VISIT_NO = tblNLA_WaterQualityData.VISIT_NO) AND (NLA2007Sites_DesignInfo.SITE_ID = tblNLA_WaterQualityData.SITE_ID)) INNER JOIN NLA_DIATOM_INFERRED_CHEM ON (NLA2007Sites_DesignInfo.VISIT_NO = NLA_DIATOM_INFERRED_CHEM.VISIT_NO) AND (NLA2007Sites_DesignInfo.SITE_ID = NLA_DIATOM_INFERRED_CHEM.SITE_ID)
...
#Field Definitions:
  ...
  #ZMAX (m): max depth from NLA data
...
#Calculated Fields
    ...
    VolumeNLA=ZMAX*Area/3#(m3) Estimated volume using from NLA MaxDepth prediction. Formula=volumne of cone.
...
#split Zmax into High Med & Low values
        Zmax<-cut(ZMAX,quantile(ZMAX,(0:4)/4,na.rm=T))
        levels(Zmax)<-c("Low","Med","Med","High")
        table(Zmax)
        #summary(ZMAX[Zmax=="High"])
...
#Compare Observed NLA with Sparrow     
#windows(record=T)  
#plot.new()
win.graph(10, 7.5)  
par(mfrow=c(2,3))
...
plot(LM$model[,2],LM$model[,1],pch=19,
  ...
  abline(sumLM,lwd=2,col="green")
    ...
    text(min(LM$model)+.5,max(LM$model),paste('adjR2=',round(sumLM$adj.r.squared,3)))
    text(min(LM$model)+.5,max(LM$model)-.25,paste('rmse=',round(rmse,3)))
plot(LM$model[,2],LM$residuals,pch=19,
  xlab=paste('MRB1 ',names(LM$model[2])),ylab=paste('NLA ',names(LM$model[1]),' Residuals'))
  abline(h=0,lwd=2,col="blue")
  title(main = 'Residuals')
plot(LM$fitted.values,LM$model[,1],pch=19,
        xlab=paste('MRB1 Predicted',names(LM$model[1])),
        ylab=paste('NLA Observed ',names(LM$model[1])),
        xlim=c(min(LM$model),max(LM$model)),ylim=c(min(LM$model),max(LM$model)))
...
group<-data.frame(HRT=HRT[!is.na(Pdia)])
     LM<-lm(log10(Pdia)~log10(Pout))
sumLM<-summary(LM)
rmse<-sqrt(sum(LM$residuals**2)/length(LM$residuals))
...
Colors<-c("red","darkgoldenrod1","green")
#group<-c("High") #use this if there are no subgroups.
group<-data.frame(HRT)
In<-data.frame(Pdia,Pout,hrtNLA,zNLA)
...
plot(log10(X[group=="High"]),log10(Y[group=="High"]),pch=19,col=Colors[1],
        xlab=paste('MRB1 Log10',names(In[2])),
        ylab=paste('NLA Observed Log10',names(In[1])),
        xlim=Lim,ylim=Lim)
  ...
  points(log10(X[group=="Low"]),log10(Y[group=="Low"]),pch=19,col=Colors[3])
    ...
    text(min(LM$model)+.5,max(LM$model),paste('rmse=',round(rmse,3)))
...
plot(Phat[group=="High"],log10(Y[group=="High"]),pch=19,col=Colors[1],
        xlab=paste('MRB1 Predicted',names(In[2])),
        ylab=paste('NLA Observed ',names(In[2])),
        xlim=Lim,ylim=Lim)
...
abline(test, lwd=2)
title(main = paste("log(Pout/(1+(",round(nl$coefficients[1],2),"hrt^",round(nl$coefficients[2],2),"*z^",round(nl$coefficients[3],2),"*Pout^",round(nl$coefficients[4],2),"*hrt)))"),cex.main=.8, 
...
plotLMH=function(x,y,group,Title,size,hline){
    plot(x[group=="High"],y[group=="High"],col=Colors[1],pch=19,cex=size,
    xlim=XLIM,ylim=YLIM,
    main=Title,xlab="SPARROW",ylab="NLA",axes=T)
    points(x[group=="Med"],y[group=="Med"],col=Colors[2],pch=19,cex=size)
    points(x[group=="Low"],y[group=="Low"],col=Colors[3],pch=19,cex=size)
