...
FROM (((MRB1_WBIDLakes INNER JOIN tblJoinNLAID_WBID ON MRB1_WBIDLakes.WB_ID = tblJoinNLAID_WBID.WB_ID) INNER JOIN tblSparrowLoads ON MRB1_WBIDLakes.WB_ID = tblSparrowLoads.WB_ID) INNER JOIN tblGISLakeVolume ON MRB1_WBIDLakes.WB_ID = tblGISLakeVolume.WBID) INNER JOIN tblNLA_WaterQualityData ON tblJoinNLAID_WBID.NLA_ID = tblNLA_WaterQualityData.SITE_ID
...
abline(test, lwd=2)
title(main = paste("log(Pin)/(1+(",round(estimate$coefficients[1],2),"hrt))"),
sub=paste('r-squared=',round(test$r.squared,4)))
...
plot(Phat, log10(TP), xlab="Estimated Log10 TP", ylab="NLA Measured Log10 TP")
abline(test, lwd=2)
title(main = paste("log(Pin)/(1+",round(estimate$coefficients[1],2),")"),
sub=paste('r-squared=',round(test$r.squared,4)))

#B&B2008 H3  Phat=log10(Pin)/(1+((5.1/MeanDepth)*hrt))
estimate <- nlrob(log10(TP) ~ log10(Pin)/(1+((c1/MeanDepth)*hrt)),
                     start=list(c1 =5.1),
                     data=MRB1,algorithm = "default",  trace=T) 
#Predict log Total Phosporus (Phat) from Sparrow Inflow Concentration
Phat=log10(Pin)/(1+((estimate$coefficients[1]/MeanDepth)*hrt))
test=summary(lm(log10(TP)~Phat))
plot(Phat, log10(TP), xlab="Estimated Log10 TP", ylab="NLA Measured Log10 TP")
abline(test, lwd=2)
title(main = paste("log(Pin)/(1+(",round(estimate$coefficients[1],2),"/MeanDepth)*hrt))"),
sub=paste('r-squared=',round(test$r.squared,4)))
...
plot(Phat, log10(TP), xlab="Estimated Log10 TP", ylab="NLA Measured Log10 TP")
abline(test, lwd=2)
title(main = paste("log(Pin)/(1+(",round(estimate$coefficients[1],2),"hrt^",round(estimate$coefficients[2],2),")"),
sub=paste('r-squared=',round(test$r.squared,4)))

#B&B2008 H5  Phat=(.65*log10(Pin))/(1+(.03*hrt))
estimate <- nlrob(log10(TP) ~ (c1*log10(Pin))/(1+(c2*hrt)),
                     start=list(c1 =.65,c2=.03),
                     data=MRB1,algorithm = "default",  trace=T) 
#Predict log Total Phosporus (Phat) from Sparrow Inflow Concentration
Phat=(estimate$coefficients[1]*log10(Pin))/(1+(estimate$coefficients[2]*hrt))
test=summary(lm(log10(TP)~Phat))
plot(Phat, log10(TP), xlab="Estimated Log10 TP", ylab="NLA Measured Log10 TP")
abline(test, lwd=2)
title(main = paste("(",round(estimate$coefficients[1],2),"*log(Pin))/(1+(",round(estimate$coefficients[2],2),"hrt",")"),
sub=paste('r-squared=',round(test$r.squared,4)))
