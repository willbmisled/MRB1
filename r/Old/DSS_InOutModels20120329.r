v='DSS_InOutModels20120329.r' #version = rscript file name
...
#split HRT into High Med & Low values
        DSS$HRT<-cut(DSS$hrt,quantile(DSS$hrt,(0:4)/4,na.rm=T),include.lowest=T)
        levels(DSS$HRT)<-c("Low","Med","Med","High")
        #table(DSS$HRT)
...
temp1<-data.frame(Pin=10^seq(-8,3,by=.01),hrt=median(DSS$hrt,na.rm=T),
      Zmean=median(DSS$Zmean,na.rm=T))
plot(log10(temp1$Pin),predict(nlp,newdata=temp1),pch=19,col='blue')

summary(DSS$hrt)
temp1<-data.frame(Pin=median(DSS$Pin,na.rm=T),hrt=c(0:100),
      Zmean=median(DSS$Zmean,na.rm=T))
plot(log10(temp1$hrt),predict(nlp,newdata=temp1),pch=19,col='blue')

summary(DSS$Zmean)
temp1<-data.frame(Pin=median(DSS$Pin,na.rm=T),hrt=median(DSS$hrt,na.rm=T),
      Zmean=c(0:21))
