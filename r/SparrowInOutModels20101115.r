v='SparrowInOutModels20101115.r' #version = rscript file name
...
PlotRaw<-function(X,Y,Label,Title){
  ...
  #Get axis limits & Labels
    ...
    Xlabel<-bquote('MRB1 Predicted '*log[10]*.(Label)*' mg/l')
...
win.graph();PlotRaw(log10(Nout),log10(TN),' TN','a) Total Nitrogen: NLA vs. MRB1') 
win.graph();PlotRaw(log10(Pout),log10(TP),' TP','b) Total Phosphorus: NLA vs. MRB1') 
#######################
