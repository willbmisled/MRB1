...
MRB1<-data.frame(get)
close(con)
#replace NAs with zeros for load values
xsub<-MRB1[,31:76]
xsub[is.na(xsub)]<-0
MRB1[,31:76]<-xsub
...
hist(P_Inc_Load/P_Load_Total)
hist(P_Upstream/P_Load_Total)

str(P_Inc_Load[P_Inc_Load/P_Load_Total>.5])
str(P_Inc_Load[P_Inc_Load/P_Load_Total<.5])
