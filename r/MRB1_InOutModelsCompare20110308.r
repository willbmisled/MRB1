v='MRB1_InOutModelsCompare20110308.r'
...
model<-'H6a'
nl<-c();rmse<-NA;aic<-NA;  #reset values
nl<- nlrob(log10(Y) ~ c4*log10(X/(1+(c1*hrt**c2*Z**c3*X**.5))),
  start=list(c1 = 12.26, c2 = .45, c3=-.16,c4=1),
  data=In1,algorithm = "default",  trace=T,na.action = na.exclude)
    rmse<-sqrt(sum(na.exclude(nl$residuals**2))/length(na.exclude(nl$residuals)))
    aic<-AIC(nl)
    Yhat=predict(nl, newdata = In1)
...
model<-'H6b'
nl<-c();rmse<-NA;aic<-NA;  #reset values
nl<- nlrob(log10(Y) ~ c4*log10(X/(1+(12.26*hrt**.45*Z**-.16*X**.5))),
  start=list(c4=1),
  data=In1,algorithm = "default",  trace=T,na.action = na.exclude)
    rmse<-sqrt(sum(na.exclude(nl$residuals**2))/length(na.exclude(nl$residuals)))
    aic<-AIC(nl)
