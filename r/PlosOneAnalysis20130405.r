#R script "PlosOneAnalysis20130405.r" to replicate analyses and figures for publication:  
...
load(file='L:/Public/Milstead_Lakes/RData/PlosOne20121129.rda') #NOTE: this will need to be updated to the PlosOne URL
...
PlotLM<-function(LM,Label,Title,AXES){
  ...
  #Plot model values
    ...
    title(main=Title,cex.main=1.5)
    legend("bottomright",c("Short","Med","Long"),pch=19,cex=1.5,col=Colors,bty='y',title='HRT')
      R2 <- bquote(paste(adjR^2==.(round(summary(lm(LM$model[,1]~Yhat))$adj.r.squared,3))))
      Count<- bquote(paste(N==.(length(na.exclude(LM$model[,1])))))
    legend('topleft', do.call("expression",list(R2,Count)), bty = 'n',cex=1.2)
}
...
PlotNL<-function(nl,X,Y,Label,Title,AXES){
 ...
 #Get axis limits & Labels
  ...
  #Plot model values
    ...
    abline(0,1,lwd=2,col="grey")
    title(main=Title,cex.main=1.5)
      R2 <- bquote(paste(adjR^2==.(round(summary(lm(log10(Y)~Yhat))$adj.r.squared,3))))
      Count<- bquote(paste(N==.(length(na.exclude(Y)))))
    legend('topleft', do.call("expression",list(R2,Count)), bty = 'n',cex=1.2)
    legend("bottomright",c("Short","Med","Long"),pch=19,cex=1.5,col=Colors,bty='y',title='HRT')
}
...
win.graph()
par(mai=Mai)
PlotNL(nln,Nin,TN,'TN','a) Total Nitrogen: Vollenweider Model','T') 
#Figure 4b: Phosphorus
win.graph()
par(mai=Mai)
PlotNL(nlp,Pin,TP,'TP','b) Total Phosphorus: Vollenweider Model','F')  
axis(1,at=c(.002,.006,.020,.06,.2,.6,2),cex.axis=1.2)
axis(2,at=c(.002,.006,.020,.06,.2,.6,2),cex.axis=1.2)
box()
...
title(paste('a) Cumulative Distribution Function for ',Nutr),cex.main=1.3) 
...
legend("bottomright",c("SPARROW Prediction","Vollenweider Adjustment", "NLA Observed","NLA 95% c.i."),
        lty=c(1,1,1,NA),lwd=3,pch=c(NA,NA,NA,22),pt.bg = Color,cex=1.3,pt.cex = 3,col=Color[c(2,3,1,4)],bty='n') 
title(paste('b) Cumulative Distribution Function for ',Nutr),cex.main=1.3) 
########################################  
...
plot(hrt[!is.na(TN)],LMN$residuals,pch=19,col=group[!is.na(TN)],xlim=c(0,5),ylim=c(-.7,.8),
  ...
  main='a) Total Nitrogen: Linear Model Residuals',cex=1.3,cex.lab=1.5,cex.axis=1.1,cex.main=1.3)
...
plot(hrt,nln$residuals,pch=19,col=group,xlim=c(0,5),ylim=c(-.7,.8),
  xlab='Hydraulic Residence Time (years)',ylab='Residuals',
  main='b) Total Nitrogen: Vollenweider Model Residuals',cex=1.3,cex.lab=1.5,cex.axis=1.1,cex.main=1.3)
  abline(h=0,lwd=2,col="grey")
...
plot(hrt[!is.na(TP)],LMP$residuals,pch=19,col=group[!is.na(TP)],xlim=c(0,5),ylim=c(-1.2,1.4),
  xlab='Hydraulic Residence Time (years)',ylab='Residuals',
  main='c) Total Phosphorus: Linear Model Residuals',cex=1.3,cex.lab=1.5,cex.axis=1.1,cex.main=1.3)
  abline(h=0,lwd=2,col="grey")
  legend("topright",c("Short","Med","Long"),pch=19,cex=1,col=Colors,bty='n')
...
plot(hrt,nlp$residuals,pch=19,col=group,xlim=c(0,5),ylim=c(-1.2,1.4),
  xlab='Hydraulic Residence Time (years)',ylab='Residuals',
  main='d) Total Phosphorus: Vollenweider Model Residuals',cex=1.3,cex.lab=1.5,cex.axis=1.1,cex.main=1.3)
  abline(h=0,lwd=2,col="grey")
  legend("bottomright",c("Short","Med","Long"),pch=19,cex=1.0,col=Colors,bty='n')
  text(2.75,1.2,'Model UNDER-estimates Phosphorus')
  text(2.75,-1,'Model OVER-estimates Phosphorus')
###############################
...
#cols   'DarkBlue','#LtBlue','LtYellow','DarkYellow')  
...
plot(MRB1shp,col=GetColors(predTSN),pch=19,cex=.2);title(main='b) Vollenweider Adjusted Nitrogen')
plot(MRB1shp,col=GetColors(obsTSP),pch=19,cex=1);title(main='c) NLA Observed Phosphorus')
plot(MRB1shp,col=GetColors(predTSP),pch=19,cex=.2);title(main='d) Vollenweider Adjusted Phosphorus')
legend('bottomright',levels(obsTSN),pch=19,col=cols,bty='n',cex=1.20)
