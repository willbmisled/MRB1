...
#From SPARROW
  PRmrb1<-1-(MRB1$Poutput/MRB1$Pinput) 
    summary(round(100*PRmrb1,1))    #median=11% Removed
  sum(MRB1$Pinput*PRmrb1)/10**6 # .67 M kilos P removed by NE Lakes

#From Reckhow model
  PoutputEst<-MRB1$TPest*10**-6*MRB1$Flow
  PRnl<-(MRB1$Pinput-PoutputEst)/MRB1$Pinput 
  PRnl<-1-(PoutputEst/MRB1$Pinput) 
    summary(round(100*PRnl,1))   #median=36.10% Removed
  sum(MRB1$Pinput*PRnl)/10**6 # 11.21 M kilos P removed by NE Lakes
...
#Calculate P_PR(mg/l) = Pin-Phosphorus removed based on Harrison et al 2009
  ...
  MRB1$Pinput[MRB1$Pinput==0]
  PRmrb1[PRmrb1==0]         breaks=seq(-5,5,by=1)
...
hist(log10(MRB1$Pinput*PRmrb1),xlim=c(-5,5),breaks=seq(-6,6,by=1),ylim=c(0,9000))
