...
nl<- nlrob(log10(TP) ~ log10(Pin/(1+((c1*(hrt**c2)*(Zmean**c3)*(Pin**c4))*hrt))),
...
LM<-lm(log10(TP)~log10(Pout),data=NLA)
...
nl<- nlrob(log10(TN) ~ log10((Nin)/(1+(c1*hrt**c2))),
...
#Predict TP for MRB1 lakes from LM
LM<-lm(log10(TN)~log10(Nout),data=NLA)
...
TS_TN<-assignTS(TN,1400,750,350)
TS_TP<-assignTS(TP,50,25,10)
TS_CHLA<-assignTS(CHLA,50,25,10)

Secchi<--SECMEAN  #need to reverse sign
TS_Secchi<-assignTS(Secchi,4,201,.7)
